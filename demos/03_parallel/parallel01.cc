...
void callback(const ParallelWork *pwork) {
  for (size_t i = 0; i < pwork->size(); i++) {
    auto ctx = static_cast<tutorial_series_context *>(
        pwork->series_at(i)->get_context());
...
int main() {
  ...
  std::vector<std::string> url_list = {"www.baidu.com", "www.cn.bing.com"/*,
                                       "www.github.com"*/};
  for (auto &url : url_list) {
    ...
    auto task = WFTaskFactory::create_http_task(
        url, REDIRECT_MAX, RETRY_MAX, [](WFHttpTask *http_task) {
          // 保存和使用抓取结果
          // 把抓取结果保存在自己的series context里，以便并行任务获取。
          // 这个做法是必须的，因为http任务在callback之后就会被回收，我们只能把resp通过std::move()操作移走。
          // 在上面callback中能很容易的获取
          LOG_DEBUG("get url response");
          auto ctx = static_cast<tutorial_series_context *>(
              series_of(http_task)->get_context());
