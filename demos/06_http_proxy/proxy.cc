...
// 本质上是将用户请求原封不动转发到对应的web server，再将web
// server的回复原封不动转发给用户

// 浏览器发给proxy的请求里，request
// uri包含了scheme和host，port，转发时需要去除。
...
void reply_callback(WFHttpTask *proxy_task) {
  ...
  if (proxy_task->get_state() == WFT_STATE_SUCCESS)
    spdlog::info("{}: Success. Http Status: {}, BodyLength: {}", context->url,
                 proxy_resp->get_status_code(), size);
  else /* WFT_STATE_SYS_ERROR*/
    spdlog::info("{}: Reply failed: {}, BodyLength: {}", context->url,
                 strerror(proxy_task->get_error()), size);
}
...
void http_callback(WFHttpTask *task) {
  ...
  if (state == WFT_STATE_SUCCESS) {
    ...
    *proxy_resp = std::move(*resp);
    ...
    spdlog::info("{}: Fetch failed. state = {}, error = {}: {}", context->url,
                 state, task->get_error(), err_string);
...
void process(WFHttpTask *proxy_task) {
  ...
  context->url =
      req->get_request_uri();  // req->get_request_uri()调用得到浏览器请求的完整URL，通过这个URL构建发往server的http任务。
  context->proxy_task =
      proxy_task;  // context里包含了proxy_task本身，以便我们异步填写结果。
  ...
  // todo: req->get_request_uri() 获取的是 /noreply 这様的后缀，而并不是意料中的
  // https://127.0.0.1:8888/noreply
  ...
  req->set_request_uri(
      http_task->get_req()
          ->get_request_uri());  // 会将request_uri里的http://host:port部分去掉，只保留path之后的部分
  req->get_parsed_body(
      &body, &len);  // 解析下来的http body指定为向外输出的http body，无需拷贝
  req->append_output_body_nocopy(body, len);
  *http_task->get_req() = std::move(
      *req);  // req是我们收到的http请求，我们最终要通过std::move()把它直接移动到新请求上。

  /* also, limit the remote webserver response size. */
  http_task->get_resp()->set_size_limit(200 * 1024 * 1024);

  *series
      << http_task;  // 构造好http请求后，将这个请求放到当前series末尾，process函数结束。
}
