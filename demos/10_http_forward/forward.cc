...
void process(WFHttpTask *server_task) {
  ...
  // 向series里push_back一个http
  // client任务，并且把这个http任务返回的结果写进server的回复消息
  WFHttpTask *client_task =
      WFTaskFactory::create_http_task(url, 3, 2, callback);
  *client_task->get_req() = std::move(*req);  // 把req转发给后端
  client_task->get_req()->set_header_pair(
      "Host", "www.baidu.com");  // 少这个会403 forbidden

  SeriesWork *series = series_of(server_task);
  // 每个运行中的任务一点处于某个任务流（series）里，同一个series上的任务，可以通过series
  // context共享上下文。 series上下文用来存放server task,
  // 以便在client任务完成之后填写回复消息
