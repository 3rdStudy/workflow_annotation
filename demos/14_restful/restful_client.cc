...
void pread_callback(WFFileIOTask *pread_task) {
  ...
  WFHttpTask *http_task =
      WFTaskFactory::create_http_task(ctx->url, 4, 2, http_callback);
  http_task->get_req()->set_method("POST");
  http_task->get_req()->append_output_body_nocopy(args->buf,
                                                  args->count); /* nocopy */
...
WFFileIOTask *create_file_request(const std::string &url,
                                  std::string img_path) {
