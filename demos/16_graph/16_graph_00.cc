...
void http_callback(WFHttpTask *task) {
  ...
  spdlog::info("state: {}", task->get_state());
}
...
int main() {
  ...
  WFTimerTask *timer = WFTaskFactory::create_timer_task(
      1 * 1000 * 1000,
      [](WFTimerTask *) { fprintf(stderr, "timer task complete(1s).\n"); });

  /* Http task1 */
  WFHttpTask *http_task1 = WFTaskFactory::create_http_task(
      "https://www.sogou.com/", REDIRECT_MAX, RETRY_MAX, http_callback);
  http_task1->user_data = &size1;

  /* Http task2 */
  WFHttpTask *http_task2 = WFTaskFactory::create_http_task(
