...
ParallelWork *create_fetch_paralell() {
  std::vector<std::string> urls = {
      "http://www.baidu.com", "http://www.bing.com", "http://www.sogo.com"};

  ParallelWork *pwork = Workflow::create_parallel_work(parallel_callback);
  for (auto &url : urls) {
    WFHttpTask *task =
        WFTaskFactory::create_http_task(url, 4, 2, fetch_callback);
...
void process(WFHttpTask *server_task) {
  ...
  if (strcmp(req->get_request_uri(), "/test") == 0) {
    ...
    server_task->get_resp()->append_output_body(
        "<html>server other response</html>");
    return;
  }
}
