...
enum {
  ...
  WFT_STATE_SUCCESS =
      CS_STATE_SUCCESS,  // 任务成功。client接收到完整的回复，或server把回复完全写进入发送缓冲（但不能确保对方一定能收到）。
  WFT_STATE_TOREPLY = CS_STATE_TOREPLY,     /* for server task only */
  WFT_STATE_NOREPLY = CS_STATE_TOREPLY + 1, /* for server task only */
  WFT_STATE_SYS_ERROR =
      CS_STATE_ERROR,  // 系统错误。这种情况，task->get_error()得到的是系统错误码errno。
                       // 当get_error()得到ETIMEDOUT，可以调用task->get_timeout_reason()进一步得到超时原因

  WFT_STATE_SSL_ERROR =
      65,  // SSL错误。get_error()得到的是SSL_get_error()的返回值
           // 目前SSL错误信息没有做得很全，得不到ERR_get_error()的值。
           // 所以，基本上get_error()返回值也就三个可能 : SSL_ERROR_ZERO_RETURN,
           // SSL_ERROR_X509_LOOKUP, SSL_ERROR_SSL。
...
class WFMultiThreadTask : public ParallelTask {
 ...
 public:
  WFMultiThreadTask(
      Thread *const tasks[], size_t n,
      std::function<void(WFMultiThreadTask<INPUT, OUTPUT> *)> &&cb)
      : ParallelTask(new SubTask *[n], n), callback(std::move(cb)) {
    size_t i;

    for (i = 0; i < n; i++) this->subtasks[i] = tasks[i];
 ...
 protected:
  virtual ~WFMultiThreadTask() {
    ...
    while (n > 0) delete this->subtasks[--n];
...
class WFMailboxTask : public WFGenericTask {
 ...
 protected:
  virtual void dispatch() { this->WFMailboxTask::count(); }
 ...
 public:
  WFMailboxTask(void **mailbox, size_t size,
      ...
      : next(mailbox), value(size + 1), callback(std::move(cb)) {
    this->mailbox = mailbox;
  }

  WFMailboxTask(std::function<void(WFMailboxTask *)> &&cb)
      : next(&this->user_data), value(2), callback(std::move(cb)) {
 ...
 public:
  virtual void signal(void *msg) {
    ...
    *this->msgbuf = msg;
    ...
    if (this->flag.exchange(true)) this->subtask_done();
  }
