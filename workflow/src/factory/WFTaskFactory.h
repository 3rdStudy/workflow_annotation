...
// 上面WFTask是产生应用协议层的task
// 而这里的task是产生client / server task, 与协议无关
...
class WFNetworkTaskFactory {
 ...
 public:
  static T *create_client_task(TransportType type, const std::string &host,
                               unsigned short port, int retry_max,
                               std::function<void(T *)> callback);

  static T *create_client_task(TransportType type, const std::string &url,
                               int retry_max,
                               std::function<void(T *)> callback);

  static T *create_client_task(TransportType type, const ParsedURI &uri,
                               int retry_max,
                               std::function<void(T *)> callback);
...
class WFThreadTaskFactory {
 ...
 public:
  ...
  static MT *create_multi_thread_task(
