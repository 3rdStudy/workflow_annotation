...
// 计算调度方法啊，来保证每个一个ExecQueue调度的公平性
...
int Executor::request(ExecSession *session, ExecQueue *queue) {
  ...
  if (entry) {
    entry->session =
        session;  // entry对应的session，session才是需要去执行的(session->execute())
    ...
    // 如果这是第一个任务，需要去thrdpool_schedule(第一次调用需要malloc
    // buf(entry(struct __thrdpool_task_entry))) 后面就复用entry(struct
    // __thrdpool_task_entry)了
    {
