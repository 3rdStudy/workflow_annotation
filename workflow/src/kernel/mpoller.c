...
static int __mpoller_create(const struct poller_params *params,
  ...
  if (i == mpoller->nthreads) return 0;  // 都成功则返回0
...
mpoller_t *mpoller_create(const struct poller_params *params, size_t nthreads) {
  ...
  if (nthreads == 0) nthreads = 1;  // 默认至少有一个poller线程
  ...
  // 此处  offsetof(mpoller_t, poller) : 第一个元素(nthreads) 大小 (一般member
  // var 考虑 内存对齐问题) 第二个部分是nthreads 个 注明: 这里用void *
  // 完全是为了简化写法
  ...
  if (mpoller)  // 如果malloc成功
  {
    ...
    if (__mpoller_create(params, mpoller) >= 0) return mpoller;  // create成功
...
int mpoller_start(mpoller_t *mpoller) {
  ...
  for (i = 0; i < mpoller->nthreads; i++) {
    if (poller_start(mpoller->poller[i]) < 0) break;
  }

  if (i == mpoller->nthreads) return 0;
...
void mpoller_destroy(mpoller_t *mpoller) {
  ...
  for (i = 0; i < mpoller->nthreads; i++) poller_destroy(mpoller->poller[i]);
