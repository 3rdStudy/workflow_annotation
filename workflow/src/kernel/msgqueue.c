...
static size_t __msgqueue_swap(msgqueue_t *queue) {
  ...
  cnt = queue->msg_cnt;
  // 如果cnt大于最大接收的msg，那么通知put，因为大于msg_max put_list
  // wait在那里了，所以swap清空了就要唤醒生产者put
  if (cnt > queue->msg_max - 1) pthread_cond_broadcast(&queue->put_cond);
...
void msgqueue_put(void *msg, msgqueue_t *queue) {
  ...
  *link = NULL;
  ...
  // 当收到的cnt大于最大限制 且 阻塞mode(default)， 那么wait在这,
  // 等待消费者去给消费了
