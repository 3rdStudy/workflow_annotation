...
struct __poller {
  ...
  // 其中的void *参数来自于struct poller_data里的void
  // *context（注意不是poller_params里的context）。
...
static inline int __poller_create_timerfd() {
  return timerfd_create(
      CLOCK_MONOTONIC,
      0);  // 第一个参数指定时间类型，第二个设置属性，后续通过timerfd_settime进行调节
}

static inline int __poller_add_timerfd(int fd, poller_t *poller) {
  struct epoll_event ev = {
      .events = EPOLLIN | EPOLLET,  // 边沿触发，监听可读事件
      .data = {.ptr = NULL}};
  // epoll 上添加上timerfd
  return epoll_ctl(poller->pfd, EPOLL_CTL_ADD, fd, &ev);
}
...
static inline int __poller_wait(__poller_event_t *events, int maxevents,
  ...
  // timeout); Specifying a timeout of -1 causes epoll_wait() to block
  // indefinitely
...
static inline int __poller_create_timerfd() { return dup(0); }
...
static inline long __timeout_cmp(const struct __poller_node *node1,
  ...
  if (ret == 0) ret = node1->timeout.tv_nsec - node2->timeout.tv_nsec;
...
static void __poller_tree_insert(struct __poller_node *node, poller_t *poller) {
  ...
  if (first) poller->tree_first = &node->rb;
...
static inline void __poller_tree_erase(struct __poller_node *node,
                                       poller_t *poller) {
  if (&node->rb == poller->tree_first) poller->tree_first = rb_next(&node->rb);
...
static int __poller_append_message(const void *buf, size_t *n,
  ...
  *); void *(*event)(void *); void *(*notify)(void *, void *);
          };
          void *context;
          union
          {
...
static void __poller_handle_write(struct __poller_node *node,
  ...
  if (node->data.iovcnt > 0 && ret >= 0) {
    ...
    if (poller->partial_written(count, node->data.context) >= 0) return;
  }
...
static void __poller_handle_listen(struct __poller_node *node,
  ...
  while (1) {
    ...
    p = node->data.accept((const struct sockaddr *)&ss, len, sockfd,
                          node->data.context);
    if (!p) break;
...
static void __poller_handle_connect(struct __poller_node *node,
  ...
  // int value. https://stackoverflow.com/questions/21031717/so-error-vs-errno
  // You set up a non-blocking socket and do a connect() that returns
  // -1/EINPROGRESS or -1/EWOULDBLOCK. If the connection failed, the reason is
  // hidden away inside something called so_error in the socket. Modern systems
  // let you see so_error with getsockopt(,,SO_ERROR,,) ... So, if you're
  // performing asynchronous operations on sockets, you may need to use
  // SO_ERROR. In any other case, just use errno.
...
static void __poller_handle_event(struct __poller_node *node,
  ...
  if (cnt != 0) write(node->data.fd, &cnt, sizeof(unsigned long long));
...
static void __poller_set_timer(poller_t *poller) {
  ...
  if (poller->tree_first) {
    ...
    if (!node || __timeout_cmp(first, node) < 0) node = first;
  }
...
static void *__poller_thread_routine(void *arg) {
  ...
  */
  ...
  while (1) {
    ...
    // nevents 是返回的事件，fd及其事件信息被保存在 events 这个 epoll_event
    // 结构体数组中
    ...
    for (i = 0; i < nevents; i++) {
      ...
      // 这里判断if
      // (node>1)，是因为大多数情况下，都是正常的网络事件，于是只需判断一次，就能进入处理逻辑了
      if (node > (struct __poller_node *)1) {
        // 一般我们用epoll_event.events
        // 是操作系统告诉本程序该fd当前发生的事件类型。 但是下面判断 fd
        // 发生的事件类型是通过 node->data.operation 来判断的。
        ...
        switch (node->data.operation) {
          case PD_OP_READ:
            ...
            break;
          case PD_OP_WRITE:
            // 测试中发现，向对等方发送消息均是通过Communicator::send_message_sync中的writev(entry->sockfd,
            // vectors, cnt <= IOV_MAX ? cnt : IOV_MAX)来实现，
            // 那此处中的__poller_handle_write的功能是？
            // A :
            // 消息一次发得出去，就不走异步写了。这样子快。试个大一点的消息，就会进poller了。
    ...
    if (has_pipe_event) {
      if (__poller_handle_pipe(poller)) break;
    }
...
poller_t *poller_create(const struct poller_params *params) {
  ...
  poller->nodes = (struct __poller_node **)calloc(
      n, sizeof(void *));  // 分配max_open_files 这么多的nodes
  if (poller->nodes) {
    poller->pfd =
        __poller_create_pfd();  // !!!这里终于到底了了，去调用了epoll_create(1);
    if (poller->pfd >= 0) {
...
int poller_start(poller_t *poller) {
  ...
  if (__poller_open_pipe(poller) >= 0) {
    ...
    if (ret == 0) {
      poller->tid = tid;
      poller->nodes[poller->pipe_rd] =
          POLLER_NODE_ERROR;  // todo : 设置这个干嘛
...
/**
 * @brief
 * poller_add向poller里添加一个fd，fd必须是nonblocking的，否则可能堵死内部线程。
 * fd的操作由struct poller_data的operation决定。
 *
 * @param data
 * @param timeout t表示毫秒级的超时（-1表示无限，同epoll风格）。
 * 					达到这个超时时间，fd以ERROR状态从callback返回，错误码（struct
 * poller_result里的error）为ETIMEDOUT。
 * @param poller
...
int poller_add(const struct poller_data *data, int timeout, poller_t *poller) {
  ...
  if (node) {
    ...
    node->removed = 0;
    node->res = res;
    if (timeout >= 0) __poller_node_set_timeout(timeout, node);
 ...
 * @param poller
 * @return int
 * 返回0表示成果，返回-1表示失败。如果fd不存在，返回-1且errno==ENOENT。
 ...
 * @param poller
 * @return int
 * 返回0表示成功，返回-1表示失败。返回-1且errno==ENOENT时，表示fd不存在。
 */
