...
static void *__thrdpool_routine(void *arg) {
  ...
  */
  ...
  // todo : 退出细节还需要画画图
  if (--pool->nthreads == 0) pthread_cond_signal(pool->terminate);

  pthread_mutex_unlock(
      &pool->mutex);  // todo:
                      // 这里如何多了个unlock，之前又是哪里lock的呢，理一下这个
...
static void __thrdpool_terminate(int in_pool, thrdpool_t *pool) {
  ...
  while (pool->nthreads > 0) pthread_cond_wait(&term, &pool->mutex);
...
static int __thrdpool_create_threads(size_t nthreads, thrdpool_t *pool) {
  ...
  if (ret == 0) {
    if (pool->stacksize) pthread_attr_setstacksize(&attr, pool->stacksize);
