...
int WFServerBase::start(const struct sockaddr *bind_addr, socklen_t addrlen,
  ...
  // 先初始化一些关于连接的参数配置，包含超时时间，ip地址
  if (this->init(bind_addr, addrlen, cert_file, key_file) >= 0) {
    // 使用调度器将 socket 绑定到对应的地址，即创建监听端口
    // scheduler 相当于是对 Communicator 的另一层封装，关于网络的处理都在
    // Communicator 中。
...
static int __get_addr_bound(int sockfd, struct sockaddr *addr, socklen_t *len) {
  ...
  if (getsockname(sockfd, addr, len) < 0) return -1;

  family = addr->sa_family;
  addr->sa_family = 0;
  for (i = 0; i < *len; i++) {
    if (((char *)addr)[i]) break;
  }
...
int WFServerBase::serve(int listen_fd, const char *cert_file,
  ...
  if (__get_addr_bound(listen_fd, (struct sockaddr *)&ss, &len) < 0) return -1;
...
void WFServerBase::shutdown() { this->scheduler->unbind(this); }
